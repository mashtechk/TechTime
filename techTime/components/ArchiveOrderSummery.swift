//  ArchiveOrderSummery.swift
//  techTime
//  Created by 图娜福尔 on 2020/11/5.

import SwiftUI

struct ArchiveOrderSummery: View {
    ...
    func changePayroll() {
        ...
        for (index, item) in data.archievePeriod.order_list.enumerated() {
            if data.archievePeriod.order_list[index].id == order.id {
                data.archievePeriod.order_list[index].payroll_match = paymacth
                print(item)
                break;
            }
        }
        for (index, item) in data.histories.enumerated() {
            if data.histories[index].id == data.archievePeriod.id {
                for (indexx, itemm) in data.histories[index].order_list.enumerated() {
                    if data.histories[index].order_list[indexx].id == order.id {
                        data.histories[index].order_list[indexx].payroll_match = paymacth
                        print(itemm)
                        print(item)
                        break;
                    }
